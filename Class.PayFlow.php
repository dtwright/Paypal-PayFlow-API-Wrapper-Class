    ...
    */
    ...
    private function setupDefaults() {
        $defaults = array(
            ...
            'USER'=>$this->user,
            'PWD'=>$this->password,
            'CUSTIP'=> isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '74.123.74.76',
            'VERBOSITY'=>'MEDIUM',
        );
    ...
    public function setProfilePayPeriod($profilePayPeriod = 'MONT') {
        $profilePayPeriod = strtoupper($profilePayPeriod);
        if(substr($profilePayPeriod,0,5) == 'DAYS=') {
            // passed as DAYS=X, which needs to be sent to the API as 'DAYS','FREQUENCY=X'
            //
            // plain 'DAYS' is interpreted as 'DAYS','FREQUENCY=1' so we don't worry about
            // that case.
            $parts = explode('=',$profilePayPeriod);
            $period = array(
                'PAYPERIOD'=>'DAYS',
                'FREQUENCY'=>$parts[1]
            );
     ...
     */
    public function setAmount($amount = 0, $wholeAmt = false) {
        $amt = array(
    ...
    /**
     * @uses            Sets the amount for an initial charge/transaction on recurring billing setups. Up to 15 digits with a decimal point (no dollar symbol)
     * @access        Public
     * @param         String/Integer/Float - $amount - 150.00.
     * @param         Boolean - $wholeAmt - True to remove decimal valuesfalse, to keep it.
     * @return        None.
     * @example     $PayFlow->setInitialAmount(150.00);
     */
    public function setInitialAmount($amount = 0, $wholeAmt = false) {
        if($amount == 0) {
            unset($this->NVP['OPTIONALTRX']);
            unset($this->NVP['OPTIONALTRXAMT']);
        } else {
            $amt = array(
                'OPTIONALTRX'=>'S',
    ...
    /**
     * @uses          Requests and initial Authorization transaction to verify card when starting recurring billing
     * @access        Public
     * @param         Boolean - $do_auth - true to do the auth, false to remove it from NVP
     * @return        None.
     * @example       $PayFlow->setInitialAuth(true);
     */
    public function setInitialAuth($do_auth = false) {
        if(!$do_auth) {
            unset($this->NVP['OPTIONALTRX']);
        } else {
            $auth = array(
        ...
        $this->NVP = array_merge($this->NVP, $auth);
    }
